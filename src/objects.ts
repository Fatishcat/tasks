...
export function isCorrect(question: Question, answer: string): boolean {
    const SanitizedA = answer.toLowerCase().trim();
    const SanitizedQ = question.expected.toLowerCase().trim();
    return SanitizedA === SanitizedQ;
}
...
export function isValid(question: Question, answer: string): boolean {
    ...
    if (question.type === "multiple_choice_question") {
        const isAnOption = questionOptions.some(
...
export function toMarkdown(question: Question): string {
    const multipleChoice: string =
        question.type === "multiple_choice_question" ?
            "\n- " + question.options.join("\n- ")
        :   "";
    return "# " + question.name + "\n" + question.body + multipleChoice;
}
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    ...
    const copyOf = { ...oldQuestion, id: id, name: newName, published: false };
    return copyOf;
}
